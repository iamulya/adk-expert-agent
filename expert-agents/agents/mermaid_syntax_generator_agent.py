"""
This module defines the Mermaid Syntax Generator Agent.

This is a highly specialized LLM-based agent whose sole purpose is to take a
natural language description of a diagram and generate the corresponding
Mermaid.js syntax. It uses the ADK knowledge base to inform the structure and
content of diagrams related to ADK components.
"""
...
class MermaidSyntaxGeneratorAgentInput(BaseModel):
    """Input schema for the syntax generator agent."""
...
def mermaid_syntax_generator_instruction_provider(context: ReadonlyContext) -> str:
    """
    Dynamically generates the instruction for the syntax generator agent's LLM.

    It takes the user's diagram query, combines it with the ADK knowledge context,
    and provides a very specific prompt to the LLM to generate only a Mermaid
    ...
    if (
        ...
        if (
            ...
            try:
                # Validate and extract the user's query from the input JSON.
                input_data = MermaidSyntaxGeneratorAgentInput.model_validate_json(
                    invocation_ctx.user_content.parts[0].text
                )
                user_diagram_query = input_data.diagram_query
            except (ValidationError, json.JSONDecodeError) as e:
                logger.error(
                    f"MermaidSyntaxGeneratorAgent: Error parsing input: {e}. Raw input: {invocation_ctx.user_content.parts[0].text[:100]}"
                )
    ...
    # The instruction provides the ADK context and gives a very specific command
    # about the expected output format (a single Mermaid code block).
    instruction = f"""
...
async def mermaid_syntax_generator_after_agent_cb(
    callback_context: CallbackContext,
) -> genai_types.Content | None:
    """
    Callback that runs after the agent's LLM call.
    It cleans and validates the LLM's response to ensure it's either a valid
    Mermaid code block or a clear error message. This cleaned output becomes
    the definitive result of this agent, which is then passed back to the
    ...
    mermaid_syntax_output = (
        "Error: Could not extract Mermaid syntax from the agent's response."
    )
    if last_event and last_event.content and last_event.content.parts:
        ...
        if raw_text_output:
            try:
                ...
                if (
                    isinstance(data, dict) and "result" in data
                ):  # ADK might wrap tool-like agent output
    ...
    if not mermaid_syntax_output.strip().startswith("Error:"):
        match = re.search(
            r"(```mermaid\s*[\s\S]+?\s*```)", mermaid_syntax_output, re.DOTALL
        )
        if match:
            ...
            logger.warning(
                f"MermaidSyntaxGeneratorAgent: Output doesn't look like a Mermaid block or an error: {mermaid_syntax_output[:100]}"
            )
    ...
    logger.info(
        f"MermaidSyntaxGeneratorAgent (after_agent_callback): Returning Mermaid syntax: {mermaid_syntax_output[:100]}"
    )
